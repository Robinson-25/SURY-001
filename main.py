...
if _gpu_enabled:
    ...
    print("[SURY] GPU Acceleration is ENABLED. Offloading RAM rendering workload to GPU.")
else:
    ...
    print("[SURY] Using Balanced Low RAM GPU-Composited mode for beautiful fluid rendering.")
...
_TOOL_EXECUTOR = ThreadPoolExecutor(max_workers=8, thread_name_prefix="sury-tool")
...
if sys.platform == "win32":
    try:
        ...
        if _ctypes.windll.kernel32.GetConsoleWindow() == 0:
            ...
            print("[SURY] subprocess.Popen patched: CREATE_NO_WINDOW active")
    except Exception as _e:
        print(f"[SURY] Could not patch subprocess: {_e}")
...
TOOL_DECLARATIONS = [
    {
        "name": "SURY_ui_control",
        "description": (
            "Control total sobre la ventana principal y los widgets de la interfaz de SURY. "
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        try:
            ...
            if os.path.exists("config/vosk_model"):
                ...
                print("[SURY] Modelo Vosk cargado para Modo Suspensión.")
        except Exception as e:
            print(f"[SURY] No se pudo cargar Vosk: {e}")
    ...
    def _apply_config(self, cfg: dict):
        ...
        print("[SURY] ⚙️ Config actualizada — reconectando sesión...")
    ...
    def _fire_phrase_triggers(self, user_text: str) -> bool:
        ...
        try:
            ...
            print(f"[SURY] phrase trigger error: {e}")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            _vad_applied = True
            print("[SURY] VAD config aplicado (typed)")
        except Exception:
            pass
        ...
        if not _vad_applied:
            try:
                ...
                print("[SURY] VAD config aplicado (dict)")
            except Exception:
                print("[SURY] VAD config no aplicado")
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[SURY] 🔧 {name}  {args}")
        self.ui.set_state("THINKING")



        if name == "shutdown_jarvis":
            self.ui.write_log("SYS: Apagando SURY...")
            # Must quit from Qt main thread — signals are thread-safe
            self.ui._win._shutdown_sig.emit()
            return types.FunctionResponse(
                id=fc.id, name=name,
                response={"result": "Apagando SURY. ¡Hasta luego, MI JEFE ROBINSON!"}
            )
        ...
        print(f"[SURY] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _receive_audio(self):
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.tool_call:
                        ...
                        try:
                            ...
                            print(f"[SURY] ❌ send_tool_response failed: {tool_err}")
                            raise
        except Exception as e:
            msg  = str(e)
            ...
            if code == 1011 or "1011" in msg or "Internal error" in msg:
                ...
                print(f"[SURY] ⚡ API 1011{tool_info} — reconectando...")
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[SURY] ✅ Conectado.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: SURY en línea.")
                    ...
                    if self._first_connect:
                        ...
                        try:
                            ...
                            print(f"[SURY] VisionGuardian init error: {_vge}")
                ...
                for exc in exceptions:
                    ...
                    if "Config changed" in msg:
                        ...
                        print(f"[SURY] ⏱️ Timeout al conectar — reintentando en 1s...")
                    elif "1011" in msg or "Internal error" in msg:
                        tool_hint = self._api_1011_tool or ""
                        print(f"[SURY] ⚡ API 1011{tool_hint and ' durante '+tool_hint} — reconectando...")
                        consecutive_fails += 1
                        ...
                        print(f"[SURY] ⚠️ Modelo no disponible en esta versión de API: {msg[:120]}")
                        ...
                        print(f"[SURY] 🔄 Sesión expirada — reconectando...")
            ...
            print(f"[SURY] 🔄 Reconectando en {total:.1f}s...")
...
def main():
    ...
    if ctypes.windll.kernel32.GetLastError() == 183: # ERROR_ALREADY_EXISTS
        print("[SURY] Ya hay una instancia en ejecución. Cerrando.")
        sys.exit(0)
